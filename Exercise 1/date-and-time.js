...
(function (global) {
  ...
  var date = {},
    ...
    _formatter = {
      ...
      Z: function (d /*, formatString*/) {
        ...
        return (
          (offset > 0 ? "-" : "+") +
          ("000" + Math.abs(offset - (offset % 100) * 0.4)).slice(-4)
        );
      },
      post: function (str) {
        return str;
    ...
    _parser = {
      ...
      YY: function (str /*, formatString */) {
        ...
        result.value +=
          result.value < 70 ? 2000 : result.value < 100 ? 1900 : 0;
        return result;
      },
    ...
    return formatString.replace(
      ...
      function (token) {
        return formatter[token]
          ? formatter.post(formatter[token](d, formatString))
          : token.replace(/\[(.*)]/, "$1");
      }
    );
  };
  ...
  date.locale = function (code, locale) {
    if (locale) {
      customize(
        code,
        { res: _res, formatter: _formatter, parser: _parser },
        locale
      );
    } else if (code) {
      lang = code;
